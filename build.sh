...
DOCKER_RUN_CPU_LIMIT="0.75"
    export DOCKER_RUN_CPU_LIMIT
...
function docker_local_gem_install() {
    docker run --rm -t -i --cpu-quota="$DOCKER_RUN_CPU_LIMIT" -e GEM_HOME="$GEM_HOME" -v "$(pwd)":/srv --hostname="$DOCKER_HOSTNAME" "$DOCKERLABEL":"$DOCKER_TAGS" bundle install
}
function docker_build_site() {
    docker run --rm -t -i --cpu-quota="$DOCKER_RUN_CPU_LIMIT" -e GEM_HOME="$GEM_HOME" -v "$(pwd)":/srv --hostname="$DOCKER_HOSTNAME" "$DOCKERLABEL":"$DOCKER_TAGS" jekyll build
}
